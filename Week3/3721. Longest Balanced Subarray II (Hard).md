...
public:
    struct SegTree {
        ...
        void updateParent(int nodeIndex) {
            ...
            // because when we go to right child, left child's is already determined, so we just add the sum of left child so the lower boundary of right child wont overlap with the upper bound of left child
